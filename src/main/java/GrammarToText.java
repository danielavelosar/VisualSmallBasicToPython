...
public class GrammarToText extends MyLanguageBaseListener{
    ...
    @Override
    public void enterInicio(MyLanguageParser.InicioContext ctx) {
        System.out.print("#en Python seria asi :) \n");
        if(ctx.funcion() != null) {
            for (MyLanguageParser.FuncionContext i : ctx.funcion())
                visitFuncion(i);
        }

        if(ctx.comando() != null) {
            for (MyLanguageParser.ComandoContext i : ctx.comando())
                visitComando(i);
    ...
    @Override
    public void exitInicio(MyLanguageParser.InicioContext ctx) {
        super.exitInicio(ctx);
        System.out.print(main);
        System.out.print("#termine de revisar mi programa ^-^");
    }
    ...
    public void visitTipo(MyLanguageParser.TipoContext ctx){
        ...
        if(ctx.TKN_LEFT_PAREN() != null){
            if(funcionesCreadas.search(ctx.getParent().getChild(0).getText())==-1){
                funcionesCreadas.push(ctx.getParent().getChild(0).getText());
                //TODO: sacar al hermano de la izquierda
                crearFuncion(ctx.getParent().getChild(0).getText(), false);
    ...
    public void visitFuncion(MyLanguageParser.FuncionContext ctx){
...
public  void crearFuncion(String nombreFuncion, boolean params){
    try{
        ...
        FunctionToText listener = new FunctionToText(nombreFuncion);
